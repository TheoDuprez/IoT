...
argocd login argocd.local --insecure --username admin --password $argocd_admin_password
argocd repo add http://gitlab-webservice-default.gitlab.svc.cluster.local:8181/root/playground_application.git --username root --password $gitlab_project_access_token --insecure-skip-server-verification
